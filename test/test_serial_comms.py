...
class TestNotifications(unittest.TestCase):
    ...
    def test_callback(self):
        ...
        for test in self.tests:
            ...
            serialComms = gsmmodem.serial_comms.SerialComms(
                "-- PORT IGNORED DURING TESTS --", notifyCallbackFunc=callback
            )
            serialComms.connect()
            # Fake a notification
            ...
            while (
                len(serialComms.serial._readQueue) > 0
                or len(serialComms.serial.responseSequence) > 0
            ):
                time.sleep(0.05)
            self.assertTrue(
                callbackCalled[0], "Notification callback function not called"
            )
            serialComms.close()
...
class TestSerialException(unittest.TestCase):
    ...
    def setUp(self):
        self.mockSerial = MockSerialPackage()
        gsmmodem.serial_comms.serial = self.mockSerial
        self.serialComms = gsmmodem.serial_comms.SerialComms(
            "-- PORT IGNORED DURING TESTS --"
        )
