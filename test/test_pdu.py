...
class TestSemiOctets(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            ("15125551234", bytearray([0x51, 0x21, 0x55, 0x15, 0x32, 0xF4])),
    ...
    def test_encode(self):
        ...
        for plaintext, encoded in self.tests:
            ...
            self.assertEqual(
                result,
                encoded,
                'Failed to encode plaintext string: "{0}". Expected: "{1}", got: "{2}"'.format(
                    plaintext, [b for b in encoded], [b for b in result]
                ),
            )
        ...
        for plaintext, encoded in self.tests:
            ...
            for param in (encoded, codecs.encode(compat.str(encoded), "hex_codec")):
                ...
                self.assertEqual(
                    result,
                    plaintext,
                    'Failed to decode data. Expected: "{0}", got: "{1}"'.format(
                        plaintext, result
                    ),
                )
    ...
    def test_decodeIter(self):
        ...
        iterTests = (
            (
                "0123456789",
                9,
                iter(bytearray(codecs.decode(b"1032547698", "hex_codec"))),
            ),
        )
        for plaintext, numberOfOctets, byteIter in iterTests:
            ...
            self.assertEqual(
                result,
                plaintext,
                'Failed to decode data iter. Expected: "{0}", got: "{1}"'.format(
                    plaintext, result
                ),
            )
...
class TestGsm7(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            ...
            (
                ...
                bytearray([49, 217, 140, 86, 179, 221, 112]),
            ),
            (
                "123456789",
                bytearray(b"123456789"),
                bytearray([49, 217, 140, 86, 179, 221, 112, 57]),
            ),
            (
                "Hello World!",
                bytearray(
                ...
                bytearray([200, 50, 155, 253, 6, 93, 223, 114, 54, 57, 4]),
            ),
            (
                "[{abc}]~",
                bytearray(
                ...
                bytearray([27, 222, 6, 21, 22, 143, 55, 169, 141, 111, 211, 3]),
            ),
            (
            ...
            (
                "{åΦΓΛΩΠΨΣΘ€}",
                bytearray(
                    [27, 40, 15, 18, 19, 20, 21, 22, 23, 24, 25, 27, 101, 27, 41]
                ),
                bytearray([27, 212, 67, 50, 161, 84, 44, 23, 76, 102, 83, 222, 164, 0]),
            ),
            (
                "a[]{}€",
                bytearray([97, 27, 60, 27, 62, 27, 40, 27, 41, 27, 101]),
                bytearray([225, 13, 111, 227, 219, 160, 54, 169, 77, 25]),
            ),
        )
    ...
    def test_encode(self):
        ...
        for plaintext, encoded, septets in self.tests:
            ...
            self.assertEqual(
                result,
                encoded,
                'Failed to GSM-7 encode plaintext string: "{0}". Expected: "{1}", got: "{2}"'.format(
                    plaintext, [b for b in encoded], [b for b in result]
                ),
            )
        ...
        for plaintext, encoded, septets in self.tests:
            ...
            for param in (encoded, compat.bytearrayToStr(encoded)):
                ...
                self.assertEqual(
                    result,
                    plaintext,
                    'Failed to decode GSM-7 string: "{0}". Expected: "{1}", got: "{2}"'.format(
                        [b for b in encoded], plaintext, result
                    ),
                )
        ...
        for plaintext, encoded, septets in self.tests:
            ...
            for param in (encoded, compat.bytearrayToStr(encoded), iter(encoded)):
                ...
                self.assertEqual(
                    result,
                    septets,
                    'Failed to pack GSM-7 octets into septets for string: "{0}" using parameter type: {1}. Expected: "{2}", got: "{3}"'.format(
                        plaintext,
                        type(param),
                        [b for b in septets],
                        [b for b in result],
        ...
        for plaintext, encoded, septets in self.tests:
            ...
            for param in (septets, compat.bytearrayToStr(septets), iter(septets)):
                ...
                self.assertEqual(
                    result,
                    encoded,
                    'Failed to unpack GSM-7 septets into octets for string: "{0}". Expected: "{1}", got: "{2}"'.format(
                        plaintext, [b for b in encoded], [b for b in result]
                    ),
                )
    ...
    def test_unpackSeptets_with_limits(self):
        ...
        for plaintext, encoded, septets in self.tests:
            ...
            )  # add some garbage data (should be ignored due to numberOfSeptets being set)
            result = gsmmodem.pdu.unpackSeptets(septets, limit)
            self.assertEqual(
                result,
                encoded,
                'Failed to unpack GSM-7 septets into {0} octets for string: "{1}". Expected: "{2}", got: "{3}"'.format(
                    len(encoded), plaintext, [b for b in encoded], [b for b in result]
                ),
            )
    ...
    def test_encodeInvalid(self):
        ...
        for invalidStr in tests:
            self.assertRaises(
                ValueError, gsmmodem.pdu.encodeGsm7, invalidStr, discardInvalid=False
            )
    ...
    def test_encodeInvalidDiscard(self):
        ...
        for invalidStr, encoded in tests:
            ...
            self.assertEqual(
                result,
                encoded,
                'Failed to GSM-7 encode invalid plaintext string: "{0}". Expected: "{1}", got: "{2}"'.format(
                    invalidStr, [b for b in encoded], [b for b in result]
                ),
            )
...
class TestUcs2(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            ("あ叶葉", bytearray([0x30, 0x42, 0x53, 0xF6, 0x84, 0x49])),
    ...
    def test_encode(self):
        ...
        for plaintext, encoded in self.tests:
            ...
            self.assertEqual(
                result,
                encoded,
                'Failed to UCS-2 encode plaintext string: "{0}". Expected: "{1}", got: "{2}"'.format(
                    plaintext, [b for b in encoded], [b for b in result]
                ),
            )
    ...
    def test_decode(self):
        ...
        for plaintext, encoded in self.tests:
            ...
            self.assertEqual(
                result,
                plaintext,
                'Failed to decode UCS-2 string: "{0}". Expected: "{1}", got: "{2}"'.format(
                    [b for b in encoded], plaintext, result
                ),
            )
...
class TestSmsPduAddressFields(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            ("+9876543210", 7, b"0A918967452301", b"0A918967452301"),
            (
                "+9876543210",
                7,
                b"0A918967452301000000",
                b"0A918967452301",
            ),  # same as above, but checking read limits
    ...
    def test_decodeAddressField(self):
        for plaintext, bytesRead, hexEncoded, realHexEncoded in self.tests:
            ...
            resultValue, resultNumBytesRead = gsmmodem.pdu._decodeAddressField(
                byteIter, log=True
            )
            self.assertEqual(
                resultValue,
                plaintext,
                'Failed to decode address field data "{0}". Expected: "{1}", got: "{2}"'.format(
                    hexEncoded, plaintext, resultValue
                ),
            )
            self.assertEqual(
                resultNumBytesRead,
                bytesRead,
                'Incorrect "number of bytes read" returned for data "{0}". Expected: "{1}", got: "{2}"'.format(
                    hexEncoded, bytesRead, resultNumBytesRead
                ),
            )
    ...
    def test_encodeAddressField(self):
        for plaintext, bytesRead, hexEncoded, realHexEncoded in self.tests:
            ...
            self.assertEqual(
                result,
                expected,
                'Failed to encode address field data "{0}". Expected: "{1}", got: "{2}"'.format(
                    plaintext,
                    realHexEncoded,
                    codecs.encode(compat.str(result), "hex_codec").upper(),
                ),
            )
...
class TestSmsPduSmscFields(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            ("+9876543210", 7, b"06918967452301", b"06918967452301"),
            (
                "+9876543210",
                7,
                b"06918967452301000000",
                b"06918967452301",
            ),  # same as above, but checking read limits
    ...
    def test_decodeSmscField(self):
        for plaintext, bytesRead, hexEncoded, realHexEncoded in self.tests:
            byteIter = iter(bytearray(codecs.decode(hexEncoded, "hex_codec")))
            resultValue, resultNumBytesRead = gsmmodem.pdu._decodeAddressField(
                byteIter, smscField=True
            )
            self.assertEqual(
                resultValue,
                plaintext,
                'Failed to decode SMSC address field data "{0}". Expected: "{1}", got: "{2}"'.format(
                    hexEncoded, plaintext, resultValue
                ),
            )
            self.assertEqual(
    ...
    def test_encodeSmscField(self):
        for plaintext, bytesRead, hexEncoded, realHexEncoded in self.tests:
            ...
            self.assertEqual(
                result,
                expected,
                'Failed to encode SMSC address field data "{0}". Expected: "{1}", got: "{2}"'.format(
                    plaintext,
                    realHexEncoded,
                    codecs.encode(compat.str(result), "hex_codec").upper(),
                ),
...
class TestRelativeValidityPeriod(unittest.TestCase):
    ...
    def test_encode(self):
        for validity, tpVp in self.tests:
            ...
            self.assertEqual(
                result,
                tpVp,
                'Failed to encode relative validity period: {0}. Expected: "{1}", got: "{2}"'.format(
                    validity, tpVp, result
                ),
            )
            self.assertIsInstance(
                result,
                tpVp.__class__,
                "Invalid data type returned; expected {0}, got {1}".format(
                    tpVp.__class__, result.__class__
                ),
            )
    ...
    def test_decode(self):
        for validity, tpVp in self.tests:
            ...
            self.assertEqual(
                result,
                validity,
                'Failed to decode relative validity period: {0}. Expected: "{1}", got: "{2}"'.format(
                    tpVp, validity, result
                ),
            )
    ...
    def test_encode_validityPeriodTooLong(self):
        ...
        self.assertRaises(
            ValueError, gsmmodem.pdu._encodeRelativeValidityPeriod, validity
        )
...
class TestTimestamp(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            (
                datetime(2015, 11, 27, 0, 0, 0, tzinfo=SimpleOffsetTzInfo(0)),
                b"51117200000000",
            ),
            (
                datetime(2015, 11, 27, 0, 0, 0, tzinfo=SimpleOffsetTzInfo(2)),
                b"51117200000080",
            ),  # same as previous but with GMT+2 timezone
            (
            ...
            (
                datetime(2007, 4, 12, 23, 25, 42, tzinfo=SimpleOffsetTzInfo(-8)),
                b"7040213252242B",
            ),  # same as previous but with GMT-8 timezone
        )
    ...
    def test_encode(self):
        for timestamp, encodedHex in self.tests:
            ...
            self.assertEqual(
                result,
                encoded,
                'Failed to encode timestamp: {0}. Expected: "{1}", got: "{2}"'.format(
                    timestamp,
                    encodedHex,
                    codecs.encode(compat.str(result), "hex_codec").upper(),
                ),
    ...
    def test_decode(self):
        for timestamp, encoded in self.tests:
            ...
            self.assertEqual(
                result,
                timestamp,
                'Failed to decode timestamp: {0}. Expected: "{1}", got: "{2}"'.format(
                    encoded, timestamp, result
                ),
            )
...
class TestUdhConcatenation(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            (23, 1, 3, b"0003170301"),  # 8-bit reference
    ...
    def test_encode(self):
        for ref, number, parts, ieHex in self.tests:
            ...
            self.assertEqual(
                result,
                expected,
                'Failed to encode Concatenation Information Element; expected: "{0}", got: "{1}"'.format(
                    ieHex, codecs.encode(compat.str(result), "hex_codec").upper()
                ),
            )
            ...
            self.assertNotEqual(
                result,
                expected,
                "Modifications to UDH information element object not reflected in encode()",
            )
    ...
    def test_decode(self):
        for ref, number, parts, ieHex in self.tests:
            ...
            self.assertIsInstance(
                result,
                gsmmodem.pdu.Concatenation,
                "Invalid object type returned; expected Concatenation, got {0}".format(
                    type(result)
                ),
            )
            self.assertEqual(
                result.reference,
                ref,
                "Invalid reference; expected {0}, got {1}".format(
            ...
            self.assertEqual(
                result.number,
                number,
                "Invalid part number; expected {0}, got {1}".format(
            ...
            self.assertEqual(
                result.parts,
                parts,
                "Invalid total number of parts; expected {0}, got {1}".format(
                    parts, result.parts
                ),
            )
            # Test IE constructor with kwargs
            result = gsmmodem.pdu.InformationElement(
                iei=ieData[0], ieLen=ieData[1], ieData=ieData[2:]
            )
            self.assertIsInstance(
                result,
                gsmmodem.pdu.Concatenation,
...
class TestUdhPortAddress(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            (100, 50, b"04026432"),  # 8-bit addresses
    ...
    def test_encode(self):
        for destination, source, ieHex in self.tests:
            ...
            self.assertEqual(
                result,
                expected,
                'Failed to encode PortAddress Information Element; expected: "{0}", got: "{1}"'.format(
                    ieHex, codecs.encode(compat.str(result), "hex_codec").upper()
                ),
            )
            # Now modify some values and ensure encoded values changes
    ...
    def test_decode(self):
        for destination, source, ieHex in self.tests:
            ...
            self.assertEqual(
                result.source,
                source,
                "Invalid origin port number; expected {0}, got {1}".format(
            ...
            self.assertEqual(
                result.destination,
                destination,
                "Invalid destination port number; expected {0}, got {1}".format(
...
class TestSmsPdu(unittest.TestCase):
    ...
    def test_encodeSmsSubmit(self):
        ...
        for (
            ...
            result = gsmmodem.pdu.encodeSmsSubmitPdu(
                number, text, reference, validity, smsc, rejectDuplicates, sendFlash
            )
            self.assertIsInstance(result, list)
            self.assertEqual(
                len(result),
                1,
                "Only 1 PDU should have been created, but got {0}".format(len(result)),
            )
            self.assertIsInstance(result[0], gsmmodem.pdu.Pdu)
            self.assertEqual(
                result[0].data,
                pdu,
                'Failed to encode SMS PDU for number: "{0}" and text "{1}". Expected: "{2}", got: "{3}"'.format(
                    number,
                    text,
                    pduHex,
                    codecs.encode(compat.str(result[0].data), "hex_codec").upper(),
                ),
            )
        ...
        tests = (
            (
                b"06917228195339040B917228214365F700003130805120618005D4F29C2E03",
                {
                    ...
                    "protocol_id": 0,
                    "time": datetime(
                        2013, 3, 8, 15, 2, 16, tzinfo=SimpleOffsetTzInfo(2)
                    ),
                    "text": "Test2",
                },
            ),
            ...
            (
                b"0019000B917228001011F100003170013193008017D474BB3CA787DB70903DCC4E93D3F43C885E9ED301",  # absolute validity period
                {
                    ...
                    "validity": datetime(
                        2013, 7, 10, 13, 39, tzinfo=SimpleOffsetTzInfo(2)
                    ),
                },
            ),
            ...
            (
                b"",
                {
                    ...
                    "time": datetime(
                        2014, 5, 27, 14, 1, 48, tzinfo=SimpleOffsetTzInfo(4)
                    ),
                },
            ),
        )
            ...
            for key, value in expected.items():
                ...
                if key == "udh":
                    self.assertEqual(
                        len(result[key]),
                        len(value),
                        "Incorrect number of UDH information elements; expected {0}, got {1}".format(
                    ...
                    self.assertEqual(
                        result[key],
                        value,
                        'Failed to decode PDU value for "{0}". Expected "{1}", got "{2}".'.format(
    ...
    def test_encodeSmsSubmit_concatenated(self):
        ...
        for text, number, hexPdus in tests:
            ...
            self.assertEqual(
                len(result),
                len(hexPdus),
                "Invalid number of PDUs returned; expected {0}, got {1}".format(
                    len(hexPdus), len(result)
                ),
            )
            i = 0
            for pdu in result:
                ...
                self.assertEqual(
                    pdu.data,
                    expectedPdu,
                    'Failed to encode concatentated SMS PDU (PDU {0}/{1}). Expected: "{2}", got: "{3}"'.format(
                        i + 1,
                        len(result),
                        expectedPduHex,
                        codecs.encode(compat.str(pdu.data), "hex_codec").upper(),
                    ),
                )
                i += 1
    ...
    def test_encodeSmsSubmit_invalidValidityType(self):
        ...
        self.assertRaises(
            TypeError,
            gsmmodem.pdu.encodeSmsSubmitPdu,
            **{"number": "123", "text": "abc", "validity": "INVALID"}
        )
    ...
    def test_decode_invalidPduType(self):
        ...
        self.assertRaises(
            gsmmodem.exceptions.EncodingError, gsmmodem.pdu.decodeSmsPdu, pdu
        )
