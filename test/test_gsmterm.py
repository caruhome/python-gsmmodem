...
class TestTrie(unittest.TestCase):
    ...
    def test_len(self):
        ...
        for key, value in self.keyValuePairs:
            ...
            self.assertEqual(
                len(self.trie),
                n,
                "Incorrect trie length. Expected {0}, got {1}. Last entry: {2}: {3}".format(
                    n, len(self.trie), key, value
                ),
            )
    ...
    def test_filteredKeys(self):
        ...
        prefixMatches = (
            ("abc", [key for key in keys if key.startswith("abc")]),
        ...
        for prefix, matchingKeys in prefixMatches:
            ...
            self.assertEqual(
                len(trieKeys),
                len(matchingKeys),
                "Filtered keys length failed. Prefix: {0}, expected len: {1}, items: {2}, got len {3}, items: {4}".format(
                    prefix, len(matchingKeys), matchingKeys, len(trieKeys), trieKeys
                ),
            )
            for key in matchingKeys:
                self.assertTrue(
                    key in trieKeys,
                    "Key not in trie keys: {0}. Trie keys: {1}".format(key, trieKeys),
                )
    ...
    def test_loadAtCommands(self):
        ...
        for command, help in ATCOMMANDS:
            ...
            self.assertEqual(
                command.strip(),
                command,
                "Command has leading and/or trailing spaces: {0}".format(command),
            )

            self.assertNotEqual(
                help, None, "Command's help tuple is None: {0}".format(command)
            )
                ...
                if help[2] != None:
                    ...
                    for item in help[2]:
                        self.assertEqual(
                            len(item),
                            2,
                            "Input value item tuple length should be 2, got {0}. Command: {1}, item: {2}".format(
                                len(item), command, item
                            ),
                        )
                if help[3] != None:
                    ...
                    for item in help[3]:
                        self.assertEqual(
                            len(item),
                            2,
                            "Output value item tuple length should be 2, got {0}. Command: {1}, item: {2}".format(
                                len(item), command, item
                            ),
                        )
