...
class TestGsmModemGeneralApi(unittest.TestCase):
    ...
    def test_manufacturer(self):
        def writeCallbackFunc(data):
            self.assertEqual(
                "AT+CGMI\r",
                data,
                'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                    "AT+CGMI\r", data
                ),
            )
    ...
    def test_smsc(self):
        ...
        for test in tests:
            ...
            def writeCallbackFunc2(data):
                self.assertEqual(
                    'AT+CSCA="{0}"\r'.format(test),
                    data,
                    'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                        'AT+CSCA="{0}"'.format(test), data
                    ),
                )
            ...
            def writeCallbackFunc3(data):
                ...
                self.fail(
                    "Nothing should have been written to modem, but got: {0}".format(
                        data
                    )
                )
    ...
    def test_signalStrength(self):
        ...
        self.modem.serial.writeCallbackFunc = writeCallbackFunc
        tests = (("+CSQ: 18,99", 18), ("+CSQ:4,0", 4), ("+CSQ: 99,99", -1))
    ...
    def test_waitForNetorkCoverageNoCreg(self):
        ...
        for seq in tests:
            ...
            self.assertNotEqual(
                signalStrength,
                -1,
                '"Unknown" signal strength returned - should still have blocked',
            )
            self.assertEqual(
                seq[-1], signalStrength, "Incorrect signal strength returned"
            )

    def test_waitForNetorkCoverage(self):
        """ Tests waiting for network coverage (normal) """
        tests = (("0,2", "0,2", "0,1", 82), ("0,5", 47))
        for seq in tests:
            items = iter(seq)
        ...
        self.modem.serial.writeCallbackFunc = writeCallbackFunc2
        self.assertRaises(
            TimeoutException, self.modem.waitForNetworkCoverage, timeout=1
        )
...
class TestUssd(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tests = tests = [
            (
                "*101#",
                'AT+CUSD=1,"*101#",15\r',
                '+CUSD: 0,"Available Balance: R 96.45 .",15\r\n',
                "Available Balance: R 96.45 .",
                False,
            ),
            (
                "*120*500#",
                'AT+CUSD=1,"*120*500#",15\r',
                '+CUSD: 1,"Hallo daar",15\r\n',
                "Hallo daar",
                True,
            ),
            (
                "*130*111#",
                'AT+CUSD=1,"*130*111#",15\r',
                '+CUSD: 2,"Totsiens",15\r\n',
                "Totsiens",
                False,
            ),
            (
                "*111*502#",
                'AT+CUSD=1,"*111*502#",15\r',
                '+CUSD: 2,"You have the following remaining balances:\n0 free minutes\n20 MORE Weekend minutes ",15\r\n',
                "You have the following remaining balances:\n0 free minutes\n20 MORE Weekend minutes ",
                False,
            ),
            (
                "#100#",
    ...
    def test_sendUssd(self):
        ...
        for test in self.tests:
            ...
            self.assertEqual(
                ussd.sessionActive,
                test[4],
                "Session state is invalid for test case: {0}".format(test),
            )
            ...
            if ussd.sessionActive:
                ...
                def writeCallbackFunc2(data):
                    self.assertEqual(
                        "AT+CUSD=2\r",
                        data,
                        'Invalid data written to modem; expected "AT+CUSD=2", got: "{0}"'.format(
                            data
                        ),
                    )
    ...
    def test_sendUssd_differentModems(self):
        ...
        for ussdStr, ussdResponse in tests:
            for fakeModem in fakemodems.createModems():
                fakeModem.responses['AT+CUSD=1,"{0}",15\r'.format(ussdStr)] = [
                    '+CUSD: 2,"{0}",15\r\n'.format(ussdResponse),
                    "OK\r\n",
                ]
    ...
    def test_sendUssdExtraRelease(self):
        ...
        for test in tests:
            ...
            self.assertEqual(
                ussd.message,
                test[1],
                'Invalid message received; expected "{0}", got "{1}"'.format(
                    test[1], ussd.message
                ),
            )
            self.assertEqual(
                ussd.sessionActive, False, "Invalid session state - should be inactive"
            )
    ...
    def test_sendUssdError(self):
        ...
        self.modem.serial.responseSequence = ["ERROR\r\n"]
        self.assertRaises(
            gsmmodem.exceptions.CommandError, self.modem.sendUssd, "*101#"
        )
        ...
        tests = (
            (
                ...
                [
                    "OK\r\n",
                    0.1,
                    '+CUSD: 2,"Notification appended",15\r\n',
            ...
            (
                ...
                [
                    "Yet another random notification!\r\n",
                    "OK\r\n",
                    0.1,
                    '+CUSD: 2,"Notification before OK",15\r\n',
                ],
            ),
        )
    ...
    def test_sendUssd_responseTimeout(self):
        ...
        self.assertRaises(
            ...
            self.modem.sendUssd,
            **{"ussdString": "*101#", "responseTimeout": 0.05}
        )
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_smscPreloaded(self):
        ...
        for test in tests:
            for fakeModem in fakemodems.createModems():
                ...
                fakeModem.simBusyErrorCounter = (
                    3
                )  # Enable "SIM busy" errors for modem for more accurate testing
                FAKE_MODEM = fakeModem
                mockSerial = MockSerialPackage()
                gsmmodem.serial_comms.serial = mockSerial
                modem = gsmmodem.modem.GsmModem("-- PORT IGNORED DURING TESTS --")
                ...
                self.assertEqual(
                    test, modem.smsc, "SMSC number was changed/deleted during connect()"
                )
                modem.close()
        FAKE_MODEM = None
    ...
    def test_cfun0(self):
        ...
        for fakeModem in fakemodems.createModems():
            ...
            SERIAL_WRITE_CALLBACK_FUNC = None
            self.assertTrue(
                cfunWritten[0], "Modem CFUN setting not set to 1 during connect()"
            )
            modem.close()
            FAKE_MODEM = None
    ...
    def test_wavecomConnectSpecifics(self):
        ...
        modem.connect()
        self.assertTrue(
            gsmmodem.modem.Call.dtmfSupport,
            "+VTS in AT+CLAC response should have indicated DTMF support",
        )
        modem.close()
        FAKE_MODEM = None
    ...
    def test_huaweiConnectSpecifics(self):
        ...
        self.assertTrue(
            gsmmodem.modem.Call.dtmfSupport, "Huawei modems should have DTMF support"
        )
        modem.close()
        FAKE_MODEM = None
    ...
    def test_smscSpecifiedBeforeConnect(self):
        ...
        modem.connect()
        self.assertTrue(
            cscaWritten[0], "Preset SMSC value not written to modem during connect()"
        )
        self.assertEqual(
            modem.smsc, smscNumber, "Pre-set SMSC not stored correctly during connect()"
        )
        modem.close()
        FAKE_MODEM = None
    ...
    def test_cpmsNotSupported(self):
        ...
        SERIAL_WRITE_CALLBACK_FUNC = None
        self.assertTrue(
            cpmsWritten[0], "Modem CPMS allowed values not checked during connect()"
        )
        modem.close()
        FAKE_MODEM = None
    ...
    def test_cnmiNotSupported(self):
        ...
        SERIAL_WRITE_CALLBACK_FUNC = None
        self.assertTrue(
            cnmiWritten[0], "AT+CNMI setting not written to modem during connect()"
        )
        self.assertFalse(
            modem._smsReadSupported,
            "Modem's internal SMS read support flag should be False if AT+CNMI is not supported",
        )
        modem.close()
        FAKE_MODEM = None
    ...
    def test_clipNotSupported(self):
        ...
        SERIAL_WRITE_CALLBACK_FUNC = None
        self.assertTrue(
            clipWritten[0], "AT+CLIP=1 not written to modem during connect()"
        )
        self.assertFalse(
            crcWritten[0],
            "AT+CRC=1 should not be attempted if AT+CLIP is not supported",
        )
        self.assertFalse(
            modem._callingLineIdentification,
            "Modem's internal calling line identification flag should be False if AT+CLIP is not supported",
        )
        self.assertFalse(
            modem._extendedIncomingCallIndication,
            "Modem's internal extended calling line identification information flag should be False if AT+CLIP is not supported",
        )
        modem.close()
        FAKE_MODEM = None
    ...
    def test_crcNotSupported(self):
        ...
        self.assertTrue(
            modem._callingLineIdentification,
            "Modem's internal calling line identification flag should be True if AT+CLIP is supported",
        )
        self.assertFalse(
            modem._extendedIncomingCallIndication,
            "Modem's internal extended calling line identification information flag should be False if AT+CRC is not supported",
        )
        modem.close()
        FAKE_MODEM = None
...
class TestGsmModemDial(unittest.TestCase):
        ...
        for fakeModem in testModems:
            ...
            for number, callId, callType in tests:
                ...
                def writeCallbackFunc(data):
                    ...
                    self.assertEqual(
                        "ATD{0};\r".format(number),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}". Modem: {2}'.format(
                            "ATD{0};".format(number),
                            data[:-1] if data[-1] == "\r" else data,
                            modem,
                        ),
                    )
                ...
                self.modem.serial.responseSequence.extend(
                    modem.getPreCallInitWaitSequence()
                )
                # Fake call initiated notification
                self.modem.serial.responseSequence.extend(
                    modem.getCallInitNotification(callId, callType)
                )
                call = self.modem.dial(number)
                # Wait for the read buffer to clear
                while (
                    len(self.modem.serial._readQueue) > 0
                    or len(self.modem.serial.responseSequence) > 0
                ):
                    time.sleep(0.05)
                ...
                self.assertTrue(
                    call.active,
                    "Call state invalid: should be active. Modem: {0}".format(modem),
                )
                self.assertFalse(
                    call.answered,
                    "Call state invalid: should not yet be answered. Modem: {0}".format(
                        modem
                    ),
                )
                ...
                # Fake an answer
                self.modem.serial.responseSequence = modem.getRemoteAnsweredNotification(
                    callId, callType
                )
                ...
                self.assertTrue(
                    call.answered,
                    "Remote call answer was not detected. Modem: {0}".format(modem),
                )
                self.assertTrue(
                    call.active,
                    "Call state invalid: should be active. Modem: {0}".format(modem),
                ...
                self.assertFalse(
                    call.answered,
                    "Hangup call did not change answered state. Modem: {0}".format(
                        modem
                    ),
                )
                self.assertFalse(
                    call.active,
                    "Call state invalid: should not be active (local hangup). Modem: {0}".format(
                        modem
                    ),
                )
                ...
                self.modem.serial.responseSequence = modem.getRemoteHangupNotification(
                    callId, callType
                )
                # Wait a bit for the event to be picked up
                while (
                ...
                self.assertFalse(
                    call.answered,
                    "Remote hangup was not detected. Modem: {0}".format(modem),
                )
                self.assertFalse(
                    call.active,
                    "Call state invalid: should not be active (remote hangup). Modem: {0}".format(
                        modem
                    ),
                )
                self.assertNotIn(call.id, self.modem.activeCalls)
                ...
                self.assertFalse(
                    call.answered,
                    'Call should not have been in "answered" state. Modem: {0}'.format(
                        modem
                    ),
                )
                self.assertIn(call.id, self.modem.activeCalls)
                self.assertEqual(len(self.modem.activeCalls), 1)
                # Now reject the call
                self.modem.serial.responseSequence = modem.getRemoteRejectCallNotification(
                    callId, callType
                )
                # Wait a bit for the event to be picked up
                while (
                ...
                self.assertFalse(
                    call.answered,
                    "Call state invalid: should not be answered (remote call rejection). Modem: {0}".format(
                        modem
                    ),
                )
                self.assertFalse(
                    call.active,
                    "Call state invalid: should not be active (remote rejection). Modem: {0}".format(
                        modem
                    ),
                )
                self.assertNotIn(call.id, self.modem.activeCalls)
                ...
                for number, callId, callType in tests:
                    ...
                    def callUpdateCallbackFunc1(call):
                        ...
                        if callbackVars[2] == 0:  # Expected "answer" event
                            self.assertTrue(
                                call.active,
                                "Call state invalid: should be active. Modem: {0}".format(
                                    modem
                                ),
                            )
                            self.assertTrue(
                                call.answered,
                                "Call state invalid: should have been answered. Modem: {0}".format(
                                    modem
                                ),
                            )
                            ...
                            self.assertFalse(
                                call.answered,
                                'Call state invalid: "answered" should be false after hangup. Modem: {0}'.format(
                                    modem
                                ),
                            )
                            self.assertFalse(
                                call.active,
                                "Call state invalid: should be inactive. Modem: {0}".format(
                                    modem
                                ),
                            )
                        callbackVars[1] = True  # set "callback called" flag

                    call = self.modem.dial(
                        number, callStatusUpdateCallbackFunc=callUpdateCallbackFunc1
                    )
                    self.assertIsInstance(call, gsmmodem.modem.Call)
                    callbackVars[0] = call
                    self.assertTrue(
    ...
    def test_dialError(self):
        ...
        self.init_modem(
            fakemodems.HuaweiK3715()
        )  # Use a modem that supports call update notifications
        self.modem.serial.responseSequence = ["+CME ERROR: 30\r\n"]
...
class TestGsmModemPinConnect(unittest.TestCase):
    ...
    def test_connectPin_incorrect(self):
        ...
        self.init_modem(fakeModem)
        self.assertRaises(
            gsmmodem.exceptions.IncorrectPinError, self.modem.connect, **{"pin": "1234"}
        )
        self.modem.close()
        SERIAL_WRITE_CALLBACK_FUNC = None
    ...
    def test_connectPin_pukRequired(self):
        ...
        self.init_modem(fakeModem)
        self.assertRaises(
            gsmmodem.exceptions.PukRequiredError, self.modem.connect, **{"pin": "1234"}
        )
        self.modem.close()
        SERIAL_WRITE_CALLBACK_FUNC = None
    ...
    def test_connectPin_timeoutEvents(self):
        ...
        for response, shouldTimeout in tests:
            ...
            if shouldTimeout:
                self.assertRaises(
                    gsmmodem.exceptions.TimeoutException, self.modem.connect
                )
            else:
...
class TestIncomingCall(unittest.TestCase):
    ...
    def init_modem(self, modem, incomingCallCallbackFunc):
        ...
        self.mockSerial = MockSerialPackage()
        gsmmodem.serial_comms.serial = self.mockSerial
        self.modem = gsmmodem.modem.GsmModem(
            "-- PORT IGNORED DURING TESTS --",
    ...
    def test_incomingCallAnswer(self):
        ...
        for modem in fakemodems.createModems():
            ...
            def incomingCallCallbackFunc(call):
                try:
                    ...
                    self.assertEqual(
                        call.number,
                        callReceived[2],
                        'Caller ID (caller number) incorrect. Expected: "{0}", got: "{1}". Modem: {2}'.format(
                            callReceived[2], call.number, modem
                        ),
                    )
                    self.assertFalse(
                        call.answered,
                    ...
                    self.assertEqual(
                        call.type,
                        callReceived[1],
                        'Invalid call type; expected "{0}", got "{1}". Modem: {2}'.format(
                            callReceived[1], call.type, modem
                        ),
                    )
                    ...
                    self.assertTrue(
                        call.answered,
                        "Call state invalid: should be answered. Modem: {0}".format(
                            modem
                        ),
                    )
                    ...
                    self.assertFalse(
                        call.answered,
                        "Call state invalid: hangup did not change call state. Modem: {0}".format(
                            modem
                        ),
                    )
                    self.assertNotIn(call.id, self.modem.activeCalls)
            ...
            for number, cringParam, callType in tests:
                ...
                self.modem.serial.responseSequence = modem.getIncomingCallNotification(
                    number, cringParam
                )
    ...
    def test_incomingCallCrcNotSupported(self):
        ...
        def callbackFunc(call):
            ...
            self.assertEqual(
                call.type,
                None,
                'Invalid call type; expected "{0}", got "{1}".'.format(None, call.type),
            )
            callReceived[0] = True
        ...
        self.assertFalse(
            self.modem._extendedIncomingCallIndication,
            "Extended incoming call indicator flag should be False",
        )
    ...
    def test_incomingCallCrcChangedExternally(self):
        ...
        self.assertTrue(
            self.modem._extendedIncomingCallIndication,
            "Extended incoming call indicator flag should be True",
        )
        ...
        self.assertFalse(
            self.modem._extendedIncomingCallIndication,
            "Extended incoming call indicator flag should be False because AT+CRC=1 failed",
        )
...
class TestCall(unittest.TestCase):
    ...
    def testDtmf(self):
        ...
        for fakeModem in fakemodems.createModems():
            ...
            self.assertEqual(
                gsmmodem.modem.Call.DTMF_COMMAND_BASE,
                fakeModem.dtmfCommandBase,
                'Invalid base DTMF command for modem: {0}; expected "{1}", got "{2}"'.format(
                    fakeModem,
                    fakeModem.dtmfCommandBase,
                    gsmmodem.modem.Call.DTMF_COMMAND_BASE,
                ),
            ...
            tests = (
                ("3", "AT{0}3\r".format(fakeModem.dtmfCommandBase.format(cid=call.id))),
                (
                    "1234",
                    "AT{0}1;{0}2;{0}3;{0}4\r".format(
                        fakeModem.dtmfCommandBase.format(cid=call.id)
                    ),
                ),
                (
                    "#0*",
            ...
            for tones, expectedCommand in tests:
                ...
                def writeCallbackFunc(data):
                    expectedCommand = "AT{0}{1}\r".format(
                        fakeModem.dtmfCommandBase.format(cid=call.id),
                        tones[self.currentTone],
                    )
                    self.currentTone += 1
                    self.assertEqual(
                        expectedCommand,
                        data,
                        'Invalid data written to modem for tones: "{0}"; expected "{1}", got: "{2}". Modem: {3}'.format(
                            tones,
                            expectedCommand[:-1].format(cid=self.id),
                            data[:-1] if data[-1] == "\r" else data,
                            fakeModem,
                        ),
                    )
            ...
            call.hangup()
            self.assertRaises(
                gsmmodem.exceptions.InvalidStateException, call.sendDtmfTone, "1"
            )
            self.modem.close()
        gsmmodem.modem.Call.DTMF_COMMAND_BASE = originalBaseDtmfCommand
    ...
    def testDtmfInterrupted(self):
        ...
        self.assertRaises(
            gsmmodem.exceptions.InterruptedException, call.sendDtmfTone, "5"
        )
    ...
    def testCallAnsweredCallback(self):
        ...
        call = gsmmodem.modem.Call(
            self.modem, 1, 1, "+270000000", callStatusUpdateCallbackFunc=callbackFunc
        )
        # Answer the call "remotely" - this should trigger the callback
        call.answered = True
        self.assertTrue(
            callbackCalled[0], "Call status update callback not called for answer event"
        )
        self.modem.close()
...
class TestSms(unittest.TestCase):
    ...
    def setUp(self):
        self.tests = (
            (
                ...
                "+2782913593",
                "06917228195339040A9110325476980000313080512061800CC8329BFD06DDDF72363904",
                29,
                142,
                "SM",
            ),
            (
                ...
                datetime(2013, 3, 8, 15, 2, 16, tzinfo=SimpleOffsetTzInfo(2)),
                "+2782913593",
                "06917228195339040A91896745230100003130805120618013C8309BFD56A0DF65D0391C7683C869FA0F",
                35,
                33,
                "SM",
            ),
    ...
    def initModem(self, smsReceivedCallbackFunc):
        ...
        self.modem = gsmmodem.modem.GsmModem(
            "-- PORT IGNORED DURING TESTS --",
            smsReceivedCallbackFunc=smsReceivedCallbackFunc,
        )
        self.modem.connect()
    ...
    def test_sendSmsLeaveTextModeOnInvalidCharacter(self):
        ...
        tests = (
            (
                ...
                [
                    (
                        "00218D0A91103254769800081800480065006C006C00F300200077006F0072014200640021",
                        36,
                        141,
                    )
                ],
            ...
            (
                "+0123456789",
                "12345-010 12345-020 12345-030 12345-040 12345-050 12345-060 12345-070 12345-080 12345-090 12345-100 12345-110 12345-120 12345-130 12345-140 12345-150 12345-160-Hellò wor£d!",
                3,
                datetime(2013, 3, 8, 15, 2, 16, tzinfo=SimpleOffsetTzInfo(2)),
                "82913593",
                [
                    ...
                    (
                        "00618F0A91103254769800001A0500038F020268B556CC066B21CB6C3602747FCB03E410",
                        35,
                        143,
                    ),
                ],
            ...
            (
                "+0123456789",
                "12345-010 12345-020 12345-030 12345-040 12345-050 12345-060 12345-070 12345-080 12345-090 12345-100 12345-110 12345-120 12345-130 12345-140 12345-150 12345-160-Hello word!",
                5,
                datetime(2013, 3, 8, 15, 2, 16, tzinfo=SimpleOffsetTzInfo(2)),
                "82913593",
                [
                    ...
                    (
                        "0061910A91103254769800001905000391020268B556CC066B21CB6CF61B747FCBC921",
                        34,
                        145,
                    ),
                ],
        ...
        for number, message, index, smsTime, smsc, pdus, mem, encoding in tests:
            ...
            def writeCallbackFunc(data):
                ...
                def writeCallbackFunc3(data):
                    ...
                    self.assertEqual(
                        'AT+CSCS="{0}"\r'.format(encoding),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                            'AT+CSCS="{0}"\r'.format(encoding), data
                        ),
                    )
                ...
                def writeCallbackFunc4(data):
                    ...
                    self.assertEqual(
                        "AT+CMGS={0}\r".format(tpdu_length),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                            "AT+CMGS={0}".format(tpdu_length), data
                        ),
                    )
                ...
                def writeCallbackFuncRaiseError(data):
                    self.assertEqual(
                        self.currentPdu,
                        len(pdus) - 1,
                        "Invalid data written to modem; expected {0} PDUs, got {1} PDU".format(
                            len(pdus), self.currentPdu + 1
                        ),
                    )
                ...
                def writeCallbackFunc5(data):
                    ...
                    self.assertEqual(
                        "{0}{1}".format(pdu, chr(26)),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                            "{0}{1}".format(pdu, chr(26)), data
                        ),
                    )
                    ...
                    if len(pdus) > self.currentPdu:
                        ...
                        self.modem.serial.writeCallbackFunc = (
                            writeCallbackFuncRaiseError
                        )
            ...
            self.assertEqual(
                self.modem._smsEncoding,
                encoding,
                'Modem uses invalid encoding. Expected "{0}", got "{1}"'.format(
                    encoding, self.modem._smsEncoding
                ),
            )
            ...
            self.assertEqual(
                sms.number,
                number,
                'Sent SMS has invalid number. Expected "{0}", got "{1}"'.format(
                    number, sms.number
                ),
            )
            self.assertEqual(
                sms.text,
                message,
                'Sent SMS has invalid text. Expected "{0}", got "{1}"'.format(
                    message, sms.text
                ),
            )
            self.assertIsInstance(
                sms.reference,
                int,
                'Sent SMS reference type incorrect. Expected "{0}", got "{1}"'.format(
                    int, type(sms.reference)
                ),
            )
            ...
            self.assertEqual(
                sms.reference,
                ref,
                'Sent SMS reference incorrect. Expected "{0}", got "{1}"'.format(
                    ref, sms.reference
                ),
            )
            self.assertEqual(
                sms.status,
                gsmmodem.modem.SentSms.ENROUTE,
                'Sent SMS status should have been {0} ("ENROUTE"), but is: {1}'.format(
                    gsmmodem.modem.SentSms.ENROUTE, sms.status
                ),
            )
            ...
            self.modem._smsSupportedEncodingNames = (
                None
            )  # Force modem to ask about possible encoding names
        self.modem.close()
    ...
    def test_sendSmsTextMode(self):
        ...
        for (
            ...
            def writeCallbackFunc(data):
                def writeCallbackFunc2(data):
                    self.assertEqual(
                        "{0}{1}".format(message, chr(26)),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                            "{0}{1}".format(message, chr(26)), data
                        ),
                    )
                    self.modem.serial.flushResponseSequence = True

                self.assertEqual(
                    'AT+CMGS="{0}"\r'.format(number),
                    data,
                    'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                        'AT+CMGS="{0}"'.format(number), data
                    ),
                )
                self.modem.serial.writeCallbackFunc = writeCallbackFunc2
    ...
    def test_sendSmsPduMode(self):
        ...
        for (
            ...
            def writeCallbackFunc(data):
                ...
                def writeCallbackFunc2(data):
                    self.assertEqual(
                        "AT+CMGS={0}\r".format(calcPdu.tpduLength),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                            "AT+CMGS={0}".format(calcPdu.tpduLength), data
                        ),
                    )
                    self.modem.serial.writeCallbackFunc = writeCallbackFunc3
                    self.modem.serial.flushResponseSequence = False
                ...
                def writeCallbackFunc3(data):
                    self.assertEqual(
                        "{0}{1}".format(pduHex, chr(26)),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                            "{0}{1}".format(pduHex, chr(26)), data
                        ),
                    )
                    self.modem.serial.flushResponseSequence = True
                ...
                self.assertEqual(
                    'AT+CSCS="{0}"\r'.format(self.modem._smsEncoding),
                    data,
                    'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                        'AT+CSCS="{0}"'.format(self.modem._smsEncoding), data
                    ),
                )
                self.modem.serial.writeCallbackFunc = writeCallbackFunc2
        ...
        for (
            ...
            def writeCallbackFunc(data):
                ...
                def writeCallbackFunc2(data):
                    ...
                    self.modem.serial.responseSequence = [
                        '+ZDONR: "METEOR",272,3,"CS_ONLY","ROAM_OFF"\r\n',
                        '+ZPASR: "UMTS"\r\n',
                        "> \r\n",
                    ]
    ...
    def test_receiveSmsTextMode(self):
        ...
        def smsReceivedCallbackFuncText(sms):
            try:
                ...
                self.assertEqual(
                    sms.number,
                    callbackInfo[1],
                    'SMS sender number incorrect. Expected: "{0}", got: "{1}"'.format(
                        callbackInfo[1], sms.number
                    ),
                )
                self.assertEqual(
                    sms.text,
                    callbackInfo[2],
                    'SMS text incorrect. Expected: "{0}", got: "{1}"'.format(
                        callbackInfo[2], sms.text
                    ),
                )
                self.assertIsInstance(
                    sms.time,
                    datetime,
                    'SMS received time type invalid. Expected: datetime.datetime, got: {0}"'.format(
                        type(sms.time)
                    ),
                )
                self.assertEqual(
                    sms.time,
                    callbackInfo[4],
                    'SMS received time incorrect. Expected: "{0}", got: "{1}"'.format(
                        callbackInfo[4], sms.time
                    ),
                )
                self.assertEqual(sms.status, gsmmodem.modem.Sms.STATUS_RECEIVED_UNREAD)
                self.assertEqual(
                    sms.smsc, None, "Text-mode SMS should not have any SMSC information"
                )
            finally:
                callbackInfo[0] = True
        ...
        for (
            ...
            if tzDelta.days >= 0:
                tzValStr = "+{0:0>2}".format(
                    int(tzDelta.seconds / 60 / 15)
                )  # calculate offset in 0.25 hours
            if tzDelta.days < 0:  # negative
                tzValStr = "-{0:0>2}".format(
                    int((tzDelta.days * -3600 * 24 - tzDelta.seconds) / 60 / 15)
                )
            ...
            def writeCallbackFunc(data):
                ...
                def writeCallbackFunc2(data):
                    self.assertEqual(
                        "AT+CMGR={0}\r".format(index),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                            "AT+CMGR={0}".format(index), data
                        ),
                    )
                    self.modem.serial.responseSequence = [
                        '+CMGR: "REC UNREAD","{0}",,"{1}"\r\n'.format(
                    ...
                    def writeCallbackFunc3(data):
                        self.assertEqual(
                            "AT+CMGD={0},0\r".format(index),
                            data,
                            'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                                "AT+CMGD={0}".format(index), data
                            ),
                        )
                ...
                if self.modem._smsMemReadDelete != mem:
                    self.assertEqual(
                        'AT+CPMS="{0}"\r'.format(mem),
                        data,
                        'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                            'AT+CPMS="{0}"'.format(mem), data
                        ),
                    )
                    self.modem.serial.writeCallbackFunc = writeCallbackFunc2
                else:
        ...
        def smsReceivedCallbackFuncPdu(sms):
            try:
                ...
                self.assertEqual(
                    sms.smsc,
                    callbackInfo[5],
                    'PDU-mode SMS SMSC number incorrect. Expected: "{0}", got: "{1}"'.format(
                        callbackInfo[5], sms.smsc
                    ),
                )
            finally:
                callbackInfo[0] = True
            ...
            for (
                ...
                def writeCallbackFunc(data):
                    def writeCallbackFunc2(data):
                        ...
                        self.modem.serial.responseSequence = [
                            "+CMGR: 0,{0},{1}\r\n".format(pduAddressText, tpdu_length),
                            "{0}\r\n".format(pdu),
                            "OK\r\n",
                        ]
    ...
    def test_sendSms_waitForDeliveryReport(self):
        ...
        sms = self.modem.sendSms(
            "0829200000", "Test message", waitForDeliveryReport=True
        )
        self.assertIsInstance(sms, gsmmodem.modem.SentSms)
        self.assertNotEqual(
            sms.report, None, 'Sent SMS\'s "report" attribute should not be None'
        )
        self.assertIsInstance(sms.report, gsmmodem.modem.StatusReport)
        self.assertEqual(
            sms.status,
            gsmmodem.modem.SentSms.DELIVERED,
            'Sent SMS status should have been {0} ("DELIVERED"), but is: {1}'.format(
                gsmmodem.modem.SentSms.DELIVERED, sms.status
            ),
        )
        ...
        self.assertRaises(
            ...
            self.modem.sendSms,
            **{
                "destination": "0829200000",
                "text": "Test message",
                "waitForDeliveryReport": True,
                "deliveryTimeout": 0.05,
            }
        )
        self.modem.close()
    ...
    def test_sendSms_reply(self):
        ...
        receivedSms = gsmmodem.modem.ReceivedSms(
            self.modem,
            gsmmodem.modem.ReceivedSms.STATUS_RECEIVED_READ,
            "+27820000000",
            datetime(2013, 3, 8, 15, 2, 16, tzinfo=SimpleOffsetTzInfo(2)),
            "Text message",
            "+9876543210",
    ...
    def test_sendSms_noCgmsResponse(self):
        ...
        self.assertRaises(
            gsmmodem.exceptions.CommandError,
            self.modem.sendSms,
            "+27820000000",
            "Test message",
        )
...
class TestStoredSms(unittest.TestCase):
    ...
    def initFakeModemResponses(self, textMode):
        ...
        self.expectedMessages = [
            ReceivedSms(
                ...
                "+27748577604",
                datetime(2013, 1, 28, 14, 51, 42, tzinfo=SimpleOffsetTzInfo(2)),
                "Hello raspberry pi",
                None,
            ),
            ReceivedSms(
                ...
                "+2784000153099999",
                datetime(2013, 2, 7, 1, 31, 44, tzinfo=SimpleOffsetTzInfo(2)),
            ...
            ReceivedSms(
                ...
                Sms.STATUS_RECEIVED_READ,
                "+27840001463",
                datetime(2013, 2, 7, 6, 24, 2, tzinfo=SimpleOffsetTzInfo(2)),
        ...
        if textMode:
            FAKE_MODEM.responses['AT+CMGL="REC UNREAD"\r'] = [
                '+CMGL: 0,"REC UNREAD","+27748577604",,"13/01/28,14:51:42+08"\r\n',
                "Hello raspberry pi\r\n",
                "OK\r\n",
            ]
            FAKE_MODEM.responses['AT+CMGL="REC READ"\r'] = [
            ...
            FAKE_MODEM.responses['AT+CMGL="STO UNSENT"\r'] = FAKE_MODEM.responses[
                'AT+CMGL="STO SENT"\r'
            ] = ["OK\r\n"]
            FAKE_MODEM.responses["AT+CMGL=0\r"] = FAKE_MODEM.responses[
                "AT+CMGL=1\r"
            ] = FAKE_MODEM.responses["AT+CMGL=2\r"] = FAKE_MODEM.responses[
                "AT+CMGL=3\r"
            ] = FAKE_MODEM.responses[
                "AT+CMGL=4\r"
            ] = [
                "ERROR\r\n"
            ]
        else:
            FAKE_MODEM.responses["AT+CMGL=0\r"] = [
                "+CMGL: 0,0,,35\r\n",
                "07917248014000F3240B917247587706F400003110824115248012C8329BFD06C9C373B8B82C97E741F034\r\n",
                "OK\r\n",
            ]
            FAKE_MODEM.responses["AT+CMGL=1\r"] = [
                "+CMGL: 1,1,,161\r\n",
            ...
            FAKE_MODEM.responses['AT+CMGL="REC UNREAD"\r'] = FAKE_MODEM.responses[
                'AT+CMGL="REC READ"\r'
            ] = FAKE_MODEM.responses['AT+CMGL="STO UNSENT"\r'] = FAKE_MODEM.responses[
                'AT+CMGL="STO SENT"\r'
            ] = FAKE_MODEM.responses[
            ...
            FAKE_MODEM.responses["AT+CMGR=0\r"] = [
                "+CMGR: 0,,35\r\n",
                "07917248014000F3240B917247587706F400003110824115248012C8329BFD06C9C373B8B82C97E741F034\r\n",
                "OK\r\n",
            ]
    ...
    def test_listStoredSms_pdu(self):
        ...
        self.assertEqual(
            len(messages),
            3,
            "Invalid number of messages returned; expected 3, got {0}".format(
                len(messages)
            ),
        )
        ...
        # Test filtering
        tests = (
            (Sms.STATUS_RECEIVED_UNREAD, 1),
            (Sms.STATUS_RECEIVED_READ, 2),
            (Sms.STATUS_STORED_SENT, 0),
            (Sms.STATUS_STORED_UNSENT, 0),
        )
        for status, numberOfMessages in tests:

            def writeCallbackFunc2(data):
                self.assertEqual(
                    "AT+CMGL={0}\r".format(status),
                    data,
                    'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                        "AT+CMGL={0}".format(status), data
                    ),
                )
            ...
            self.assertEqual(
                len(messages),
                numberOfMessages,
                "Invalid number of messages returned for status: {0}; expected {1}, got {2}".format(
                    status, numberOfMessages, len(messages)
                ),
            )
            del messages
        ...
        def writeCallbackFunc3(data):
            self.assertEqual(
                "AT+CMGL={0}\r".format(expectedFilter[0]),
                data,
                'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                    "AT+CMGL={0}".format(expectedFilter[0]), data
                ),
            )

            def writeCallbackFunc4(data):
                self.assertEqual(
                    "AT+CMGD={0}\r".format(expectedFilter[1][delCount[0]]),
                    data,
                    'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                        "AT+CMGD={0}".format(expectedFilter[1][delCount[0]]), data
                    ),
                )
                delCount[0] += 1
        ...
        self.modem.serial.writeCallbackFunc = writeCallbackFunc3
        messages = self.modem.listStoredSms(
            status=Sms.STATUS_RECEIVED_READ, delete=True
        )

        # Test error handling if an invalid line is added between PDU data (line should be ignored)
        self.modem.serial.writeCallbackFunc = None
        self.modem.serial.modem.responses["AT+CMGL=4\r"].insert(
            1, "AFSDLF SDKFJSKDLFJLKSDJF SJDLKFSKLDJFKSDFS\r\n"
        )
        messages = self.modem.listStoredSms()
        self.assertIsInstance(messages, list)
        self.assertEqual(
    ...
    def test_listStoredSms_text(self):
        ...
        # Test filtering
        tests = (
            (Sms.STATUS_RECEIVED_UNREAD, "REC UNREAD", 1),
            (Sms.STATUS_RECEIVED_READ, "REC READ", 2),
            (Sms.STATUS_STORED_SENT, "STO SENT", 0),
            (Sms.STATUS_STORED_UNSENT, "STO UNSENT", 0),
        )
        for status, statusStr, numberOfMessages in tests:

            def writeCallbackFunc2(data):
                self.assertEqual(
                    'AT+CMGL="{0}"\r'.format(statusStr),
                    data,
                    'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                        'AT+CMGL="{0}"'.format(statusStr), data
                    ),
                )
    ...
    def test_processStoredSms(self):
        ...
        self.assertEqual(
            i[0],
            3,
            "Message received callback count incorrect; expected 3, got {0}".format(
                i[0]
            ),
        )
        ...
        self.assertEqual(
            i[0],
            1,
            "Message received callback count incorrect; expected 1, got {0}".format(
                i[0]
            ),
        )
    ...
    def test_deleteMultipleStoredSms(self):
        ...
        for delFlag in tests:
            ...
            def writeCallbackFunc(data):
                self.assertEqual(
                    "AT+CMGD=1,{0}\r".format(delFlag),
                    data,
                    'Invalid data written to modem; expected "{0}", got: "{1}"'.format(
                        "AT+CMGD=1,{0}".format(delFlag), data
                    ),
                )
        ...
        for delFlag in tests:
            self.assertRaises(
                ValueError, self.modem.deleteMultipleStoredSms, **{"delFlag": delFlag}
            )
...
class TestSmsStatusReports(unittest.TestCase):
    ...
    def initModem(self, smsStatusReportCallback):
        ...
        self.modem = gsmmodem.modem.GsmModem(
            "-- PORT IGNORED DURING TESTS --",
            smsStatusReportCallback=smsStatusReportCallback,
        )
        self.modem.connect()
    ...
    def test_receiveStatusReportTextMode(self):
        ...
        tests = (
            (
                ...
                20,  # reference
                datetime(
                    2013, 4, 29, 19, 58, 0, tzinfo=SimpleOffsetTzInfo(1)
                ),  # sentTime
                datetime(
                    2013, 4, 29, 19, 59, 0, tzinfo=SimpleOffsetTzInfo(1)
                ),  # deliverTime
            ...
            def smsStatusReportCallbackFuncText(sms):
                try:
                    ...
                    self.assertEqual(
                        sms.status,
                        msgStatus,
                        'Status report read status incorrect. Expected: "{0}", got: "{1}"'.format(
                            msgStatus, sms.status
                        ),
                    )
                    self.assertEqual(
                        sms.number,
                    ...
                    self.assertEqual(
                        sms.reference,
                        reference,
                        'Status report SMS reference number incorrect. Expected: "{0}", got: "{1}"'.format(
                            reference, sms.reference
                        ),
                    )
                    self.assertIsInstance(
                        sms.timeSent,
                        datetime,
                        'SMS sent time type invalid. Expected: datetime.datetime, got: {0}"'.format(
                            type(sms.timeSent)
                        ),
                    )
                    self.assertEqual(
                        sms.timeSent,
                        sentTime,
                        'SMS sent time incorrect. Expected: "{0}", got: "{1}"'.format(
                            sentTime, sms.timeSent
                        ),
                    )
                    self.assertIsInstance(
                        sms.timeFinalized,
                        datetime,
                        'SMS finalized time type invalid. Expected: datetime.datetime, got: {0}"'.format(
                            type(sms.timeFinalized)
                        ),
                    )
                    self.assertEqual(
                        sms.timeFinalized,
                        deliverTime,
                        'SMS finalized time incorrect. Expected: "{0}", got: "{1}"'.format(
                            deliverTime, sms.timeFinalized
                        ),
                    )
                    self.assertEqual(
                        sms.deliveryStatus,
                        deliveryStatus,
                        'SMS delivery status incorrect. Expected: "{0}", got: "{1}"'.format(
                    ...
                    self.assertEqual(
                        sms.smsc,
                        None,
                        "Text-mode SMS should not have any SMSC information",
                    )
                finally:
                    callbackDone[0] = True
    ...
    def test_receiveStatusReportPduMode(self):
        ...
        tests = (
            (
                ...
                183,  # reference
                datetime(
                    2013, 7, 17, 15, 58, 9, tzinfo=SimpleOffsetTzInfo(2)
                ),  # sentTime
                datetime(
                    2013, 7, 17, 15, 58, 14, tzinfo=SimpleOffsetTzInfo(2)
                ),  # deliverTime
                StatusReport.DELIVERED,
            ),  # delivery status
            (
                1,
                "SM",  # This output was captured from a ZTE modem that seems to be broken (PDU is semi-invalid (SMSC length incorrect), and +CMGR output missing status)
                [
                    "+CMGR: ,,27\r\n",
                    "0297F1061C0F910B487228297020F5317062419272803170624192138000\r\n",
                    "OK\r\n",
                ...
                28,
                datetime(
                    2013, 7, 26, 14, 29, 27, tzinfo=SimpleOffsetTzInfo(2)
                ),  # sentTime
                datetime(
                    2013, 7, 26, 14, 29, 31, tzinfo=SimpleOffsetTzInfo(2)
                ),  # deliverTime
                StatusReport.DELIVERED,
            ),
        )
    ...
    def test_receiveSmsPduMode_invalidPDUsRecordedFromModems(self):
        ...
        tests = (
            (
                ...
                35,  # reference
                datetime(
                    2016, 10, 30, 4, 59, 44, tzinfo=SimpleOffsetTzInfo(8)
                ),  # sentTime
                datetime(
                    2016, 10, 30, 4, 59, 44, tzinfo=SimpleOffsetTzInfo(7)
                ),  # deliverTime
                StatusReport.DELIVERED,
            ),  # delivery status
        )
        ...
        for (
            ...
            def smsCallbackFunc1(sms):
                try:
                    ...
                    self.assertEqual(
                        sms.smsc, None, "This SMS should not have any SMSC information"
                    )
                finally:
                    callbackDone[0] = True
