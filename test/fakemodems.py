...
class FakeModem(object):
    ...
    def __init__(self):
        ...
        self.deviceBusyErrorCounter = (
            0
        )  # Number of times to issue a "Device busy" error
        ...
        if not self._pinLock and cmd == "AT+CLCC\r":
            if self._callNumber:
                if self._callState == 0:
                    return [
                        '+CLCC: 1,0,2,0,0,"{0}",129\r\n'.format(self._callNumber),
                        "OK\r\n",
                    ]
                elif self._callState == 1:
                    return [
                        '+CLCC: 1,0,0,0,0,"{0}",129\r\n'.format(self._callNumber),
                        "OK\r\n",
                    ]
                else:
                    return ["OK\r\n"]
...
class WavecomMultiband900E1800(FakeModem):
    ...
    def getResponse(self, cmd):
        ...
        if cmd == "AT+CFUN=1\r":
            self.deviceBusyErrorCounter = (
                2
            )  # This modem takes quite a while to recover from this
            return ["OK\r\n"]
...
class HuaweiK3715(FakeModem):
    ...
    def __init__(self):
        ...
        self.responses = {
            ...
            "AT+ZPAS?\r": ["ERROR\r\n"],
            "AT+CLAC\r": [
                "+CLAC:&C,&D,&E,&F,&S,&V,&W,E,I,L,M,Q,V,X,Z,T,P,\S,\V,\
...
class QualcommM6280(FakeModem):
    ...
    def __init__(self):
        ...
        self.commandsNoPinRequired = (
            []
        )  # This modem requires the CPIN command to be issued first
        self.commandsSimBusy = ["AT+CSCA?\r"]  # Issue #10 on github
        self.responses = {
            "AT+CGMI\r": ["QUALCOMM INCORPORATED\r\n", "OK\r\n"],
            "AT+CGMM\r": ["M6280\r\n", "OK\r\n"],
            "AT+CGMR\r": [
                "M6280_V1.0.0 M6280_V1.0.0 1 [Sep 4 2008 12:00:00]\r\n",
                "OK\r\n",
            ],
            "AT+CIMI\r": ["111111111111111\r\n", "OK\r\n"],
            "AT+CGSN\r": ["111111111111111\r\n", "OK\r\n"],
