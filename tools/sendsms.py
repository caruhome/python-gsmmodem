...
def parseArgs():
    ...
    parser.add_argument(
        "--debug", action="store_true", help="turn on debug (serial port dump)"
    )
    parser.add_argument(
        "destination", metavar="DESTINATION", help="destination mobile number"
    )
    parser.add_argument(
        "message",
        nargs="?",
        metavar="MESSAGE",
        help="message to send, defaults to stdin-prompt",
    )
...
def main():
    ...
    if args.lock_path is None:
        ...
        decorator = lockutils.synchronized(
            "python_gsmmodem_sendsms", external=True, lock_path=args.lock_path
        )
