...
class RawTerm(SerialComms):
    ...
    def __init__(self, port, baudrate=9600):
        super(RawTerm, self).__init__(
            port, baudrate, notifyCallbackFunc=self._handleModemNotification
        )
        self.port = port
...
class GsmTerm(RawTerm):
    ...
    def _inputLoop(self):
        ...
        actionChars = {
            self.EXIT_CHARACTER: self._exit,
    ...
    def _handleBackspace(self):
        ...
        if self.cursorPos > 0:
            ...
            self.inputBuffer = (
                self.inputBuffer[0 : self.cursorPos - 1]
                + self.inputBuffer[self.cursorPos :]
            )
            self.cursorPos -= 1
    ...
    def _handleDelete(self):
        ...
        if self.cursorPos < len(self.inputBuffer):
            self.inputBuffer = (
                self.inputBuffer[0 : self.cursorPos]
                + self.inputBuffer[self.cursorPos + 1 :]
            )
            self._refreshInputPrompt(len(self.inputBuffer) + 1)
            ...
            if inputStrLower.startswith("help"):  # help COMMAND
                ...
                if inputStrLower == "lscat":
                    ...
                    for command in self.completion:
                        sys.stdout.write(
                            "{0:<8} - {1}\n".format(
                                command, self.completion[command][1]
                            )
                        )
                    self._refreshInputPrompt(len(self.inputBuffer))
                    return
                    ...
                    if len(ls) == 2:
                        category = ls[1].lower()
                        if category in [
                            cat.lower() for cat in self.completion.categories
                        ]:
                            sys.stdout.write("\n")
                            for command in self.completion:
                ...
                if inputStrLower.startswith("at+cmgs="):
                    ...
                    try:
                        ...
                        sys.stdout.write(
                            self._color(
                                self.COLOR_YELLOW,
                                "Type your SMS message, and press CTRL+Z to send it or press ESC to cancel.\n",
                            )
                        )
    ...
    def _printGeneralHelp(self):
        ...
        sys.stdout.write(
            "{0} Press the up & down arrow keys to move backwards or forwards through your command history.\n\n".format(
                self._color(self.COLOR_YELLOW, "Command History:")
            )
        )
        sys.stdout.write(
            '{0} Press the TAB key to provide command completion suggestions. Press the TAB key after a command is fully typed (with or without a "=" character) to quickly see its syntax.\n\n'.format(
                self._color(self.COLOR_YELLOW, "Command Completion:")
            )
        )
        sys.stdout.write(
            '{0} Type a command, followed with two quesetion marks to access its documentation, e.g. "<COMMAND>??". Alternatively, precede the command with a question mark ("?<COMMAND>"), or type "help <COMMAND>".\n\n'.format(
                self._color(self.COLOR_YELLOW, "Command Documentation:")
            )
        )
        sys.stdout.write(
            '{0} Type "ls [category]" to list the available AT commands known to GSMTerm for the given category (or all commands if no category is specified).\nType "lscat" to see a list of categories.\n\n'.format(
                self._color(self.COLOR_YELLOW, "List Available Commands:")
            )
        )
        sys.stdout.write(
            '{0} Type "load <filename>" to load and execute a file containing AT commands, separated by newlines, e.g. "load ./myscript.txt".\n\n'.format(
                self._color(self.COLOR_YELLOW, "Load Script:")
            )
        )
    ...
    def _printCommandHelp(self, command=None):
        ...
        if noHelp:
            sys.stdout.write(
                "\r No help available for: {0}\n".format(
                    self._color(self.COLOR_WHITE, command)
                )
            )
        else:
            sys.stdout.write(
                "\n\n{0} ({1})\n\n".format(
                    self._color(self.COLOR_WHITE, commandHelp[1]), command
                )
            )
            sys.stdout.write(
                "{0} {1}\n".format(
                    self._color(self.COLOR_YELLOW, "Category:"), commandHelp[0]
                )
            )
            ...
            sys.stdout.write(
                "{0} {1}\n".format(
                    self._color(self.COLOR_YELLOW, "Description:"), commandHelp[4]
                )
            )
            ...
            if (
                valuesIsEnum
            ):  # "Values" is an enum of allowed values (not multiple variables); use custom label
                sys.stdout.write(
                    "{0} ".format(self._color(self.COLOR_YELLOW, commandHelp[5]))
                )
            else:
                sys.stdout.write(
                    "{0} ".format(self._color(self.COLOR_YELLOW, "Values:"))
                )
            ...
            if commandValues != None:
                ...
                for value, valueDesc in commandValues:
                    ...
                    sys.stdout.write(
                        " {0} {1}\n".format(
                            self._color(self.COLOR_MAGENTA, value),
                            valueDesc.replace("\n", "\n" + " " * (len(value) + 2))
                            if valueDesc != None
                            else "",
                        )
            ...
            if returnValues != None:
                sys.stdout.write(
                    "{0} ".format(self._color(self.COLOR_YELLOW, "Response Values:"))
                )
                sys.stdout.write("\n")
                for value, valueDesc in returnValues:
                    sys.stdout.write(
                        " {0} {1}\n".format(
                            self._color(self.COLOR_CYAN, value),
                            valueDesc.replace("\n", "\n" + " " * (len(value) + 2))
                            if valueDesc != None
                            else "",
                        )
            ...
            sys.stdout.write(
                "{0}\n {1}\n\n".format(
                    self._color(self.COLOR_YELLOW, "Command Syntax:"), "".join(syntax)
                )
            )
        self._refreshInputPrompt(len(self.inputBuffer))
    ...
    def _doCommandCompletion(self):
        ...
        if matchLen == 0 and prefix[-1] == "=":
            ...
            if matchLen == 1:
                ...
                commonPrefix = self.completion.longestCommonPrefix(
    ...
    def __printCommandSyntax(self, command):
        ...
        if commandHelp != None and len(commandHelp) > 2:
            ...
            if commandValues != None:
                ...
                displayHelp.append(
                    ("|" if valuesIsEnum else ",").join(
                        [value[0] for value in commandValues]
                    )
                )
            sys.stdout.write(
                "\r Syntax: {0}\n".format(
                    self._color(self.COLOR_WHITE, "".join(displayHelp))
                )
            )
            sys.stdout.flush()
            self._refreshInputPrompt(len(self.inputBuffer))
    ...
    def _refreshInputPrompt(self, clearLen=0):
        ...
        sys.stdout.write(
            "\r{0}{1}{2}{3}".format(
                ...
                "".join(self.inputBuffer),
                (clearLen - len(self.inputBuffer)) * " ",
                console.CURSOR_LEFT * (endPoint - self.cursorPos),
            )
        )
        sys.stdout.flush()
    ...
    def _removeInputPrompt(self):
        termPrompt = self.SMS_PROMPT if self._typingSms else self.PROMPT
        sys.stdout.write(
            "\r{0}\r".format(" " * (len(termPrompt) + len(self.inputBuffer)))
        )
