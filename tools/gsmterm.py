...
def parseArgs():
    ...
    parser = ArgumentParser(
        description="User-friendly terminal for interacting with a connected GSM modem."
    )
    parser.add_argument(
        "port",
        metavar="PORT",
    ...
    parser.add_argument(
        "-r", "--raw", action="store_true", help="switch to raw terminal mode"
    )
...
def parseArgsPy26():
    ...
    if len(args) != 1:
        parser.error(
            "Incorrect number of arguments - please specify a PORT to connect to, e.g. {0} /dev/ttyUSB0".format(
                sys.argv[0]
            )
        )
    else:
