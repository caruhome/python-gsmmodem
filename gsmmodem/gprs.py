...
class PdpContext(object):
    ...
    def __init__(
        self, cid, pdpType, apn, pdpAddress=None, dataCompression=0, headerCompression=0
    ):
...
class GprsModem(GsmModem):
    ...
    def pdpContexts(self):
        ...
        matches = allLinesMatchingPattern(
            re.compile(
                r'^\+CGDCONT:\s*(\d+),"([^"]+)","([^"]+)","([^"]+)",(\d+),(\d+)'
            ),
            cgdContResult,
        ...
        for cgdContMatch in matches:
            cid, pdpType, apn, pdpAddress, dataCompression, headerCompression = (
                cgdContMatch.groups()
            )
