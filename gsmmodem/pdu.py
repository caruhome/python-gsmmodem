...
if PYTHON_VERSION >= 3:
    ...
    unichr = chr
    toByteArray = (
        lambda x: bytearray(codecs.decode(x, "hex_codec"))
        if type(x) == bytes
        else bytearray(codecs.decode(bytes(x, "ascii"), "hex_codec"))
        if type(x) == str
        else x
    )
    ...
    toByteArray = (
        lambda x: bytearray(x.decode("hex")) if type(x) in (str, unicode) else x
    )
...
MAX_MESSAGE_LENGTH = {0x00: 160, 0x04: 140, 0x08: 70}  # GSM-7  # 8-bit  # UCS2
...
def encodeSmsSubmitPdu(
    ...
    for i in xrange(pduCount):
        ...
        if smsc:
            ...
            pdu.append(
                0x00
            )  # Don't supply an SMSC number - use the one configured in the device
        ...
        if alphabet == 0x00:  # GSM-7
            ...
            if udhLen > 0:
                shift = (
                    (udhLen + 1) * 8
                ) % 7  # "fill bits" needed to make the UDH end on a septet boundary
...
def decodeSmsPdu(pdu):
    ...
    if pduType == 0x00:  # SMS-DELIVER or SMS-DELIVER REPORT
        ...
        raise EncodingError(
            "Unknown SMS message type: {0}. First TPDU octet was: {1}".format(
                pduType, tpduFirstOctet
            )
        )
...
def _encodeRelativeValidityPeriod(validityPeriod):
    ...
    if seconds <= 43200:  # 12 hours
        ...
        raise ValueError(
            "Validity period too long; tpVp limited to 1 octet (max value: 255)"
        )
    return tpVp
...
def _decodeTimestamp(byteIter):
    ...
    return datetime.strptime(dateStr[:-2], "%y%m%d%H%M%S").replace(
        tzinfo=SmsPduTzInfo(timeZoneStr)
    )
...
def _encodeTimestamp(timestamp):
    ...
    if tzDelta.days >= 0:
        ...
        tzVal = int(
            (tzDelta.days * -3600 * 24 - tzDelta.seconds) / 60 / 15
...
def encodeTextMode(plaintext):
    ...
    if len(plaintext) > MAX_MESSAGE_LENGTH[0x00]:
        raise ValueError(
            "Message is too long for text mode (maximum {0} characters)".format(
                MAX_MESSAGE_LENGTH[0x00]
            )
        )
...
def encodeGsm7(plaintext, discardInvalid=False):
    ...
    for char in plaintext:
        ...
        if idx != -1:
            ...
            raise ValueError(
                'Cannot encode char "{0}" using GSM-7 encoding'.format(char)
            )
    return result
...
def divideTextUcs2(plainText):
    ...
    for i in range(fullChunksCount):
        result.append(
            plainText[
                i
                * MAX_MULTIPART_MESSAGE_LENGTH[0x08] : (i + 1)
                * MAX_MULTIPART_MESSAGE_LENGTH[0x08]
            ]
        )
